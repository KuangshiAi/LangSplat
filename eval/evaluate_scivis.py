...
def eval_gt_scivisdata(json_folder: Union[str, Path] = None, ouput_path: Path = None, img_paths: list = None, queries: list = None) -> Dict:
    """
    organise scivis's gt annotations
    gt format:
    ...
    return:
        ...
        or for scivis datasets without gt:
        gt_ann: dict()
            keys: str(int(idx))
            values: dict()
                keys: str(pos_query)
                values: list() which contain negative queries
    """
    if json_folder is not None and os.path.exists(json_folder):
        ...
        for js_path in gt_json_paths:
            ...
            idx = int(gt_data['info']['name'].split('_')[-1].split('.jpg')[0]) - 1 # -1 only applies for lerf_ovs dataset
        ...
        gt_ann = {}
        positive_queries = queries[0]
        negative_queries = queries[1]
        img_example = np.load(img_paths[0]) # [h, w, 3]
        h, w, _ = img_example.shape
        for img_path in img_paths:
            idx = int(os.path.basename(img_path).split('.npy')[0])
            img_ann = defaultdict(dict)
            for pos_query in positive_queries:
                img_ann[pos_query] = negative_queries
            gt_ann[f'{idx}'] = img_ann
...
def activate_stream(sem_map, 
    ...
    gt_ann: dict()
        ...
        values: dict()
            ...
            values: list() which contain negative queries
    '''
    valid_map = clip_model.get_max_across(sem_map)            # 3xkxhxw, k is the number of positive prompts
    ...
    for k in range(n_prompt):
        ...
        for i in range(n_head):
            ...
            valid_composited = colormaps.apply_colormap(p_i / (p_i.max() + 1e-6), colormaps.ColormapOptions("magma"))
            ...
            if not isinstance(next(iter(img_ann.values())), list):
...
def scivis_localization(sem_map, image, clip_model, image_name, img_ann):
...
def evaluate(feat_dir, output_path, ae_ckpt_path, img_paths, json_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims, logger, queries):
    ...
    if json_folder is not None:
        gt_ann, image_shape, image_paths = eval_gt_scivisdata(Path(json_folder), Path(output_path), img_paths, queries)
    else:
        gt_ann, image_shape, image_paths = eval_gt_scivisdata(None, Path(output_path), img_paths, queries)
    ...
    if img_paths is None:
        ...
        for j, idx in enumerate(tqdm(eval_index_list)):
            ...
            acc_num_img = scivis_localization(restored_feat, rgb_img, clip_model, image_name, img_ann)
            acc_num += acc_num_img
        ...
        for j, idx in enumerate(tqdm(eval_index_list)):
            ...
            sem_feat = compressed_sem_feats[:, j, ...]
            sem_feat = torch.from_numpy(sem_feat).float().to(device)
            rgb_img = (np.load(image_paths[j]) / 255.0).astype(np.float32)
            rgb_img = torch.from_numpy(rgb_img).to(device)
            ...
            with torch.no_grad():
                ...
                lvl, h, w, _ = sem_feat.shape
                restored_feat = model.decode(sem_feat.flatten(0, 2))
                restored_feat = restored_feat.view(lvl, h, w, -1)    # [3, h, w, 512]: 3x800x800x512
            
            img_ann = gt_ann[f'{idx}']
            clip_model.set_positives(list(img_ann.keys())) # set text labels as positive prompts
            clip_model.set_negatives(next(iter(img_ann.values()))) # set negative prompts
            
            _, _ = activate_stream(restored_feat, rgb_img, clip_model, image_name, img_ann,
                                                thresh=mask_thresh, colormap_options=colormap_options)

            _ = scivis_localization(restored_feat, rgb_img, clip_model, image_name, img_ann)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--positive_queries", nargs='+', required=True, help="List of positive texts to query")
    parser.add_argument("--negative_queries", nargs='+', default=[], help="List of negative texts to mask out")
    parser.add_argument("--test_idx", nargs='+', default=[], help="List of test indices")
    parser.add_argument("--level_idx", nargs='+', default=[], help="List of semantic level indices")
    ...
    level_idx = [int(level) for level in args.level_idx]
    feat_dir = [os.path.join(args.feat_dir, dataset_name+f"_{i}", "train/ours_None/renders_npy") for i in level_idx]
    ...
    if len(args.json_folder) != 0:
        json_folder = os.path.join(args.json_folder, dataset_name)
        img_paths = None
    else:
        json_folder = None
        if len(args.test_idx) != 0:
            img_paths = [os.path.join(args.feat_dir, dataset_name+"_1", "train/ours_None/rendersRGB_npy", f'{idx}.npy') for idx in args.test_idx]
        else:
            img_paths = sorted(glob.glob(os.path.join(args.feat_dir, dataset_name+"_1", "train/ours_None/rendersRGB_npy", '*.npy')))
    ...
    print("Querying " + args.dataset_name + " with:", args.positive_queries)
    print(f"Negative queries: {args.negative_queries}")

    evaluate(feat_dir, output_path, ae_ckpt_path, img_paths, json_folder, mask_thresh, args.encoder_dims, args.decoder_dims, logger, [args.positive_queries, args.negative_queries])